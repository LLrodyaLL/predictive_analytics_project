<template>
  ...
  <div class="search">
        ...
        <form @submit.prevent="submitForm" class="subForm">
          <div class="form-group">
            ...
            <el-input id="articul" v-model="formData.article" required/>
          </div>
          <div class="form-group">
            <label for="region">Регион</label><br>
            <el-cascader id="region" v-model="formData.region" :options="regions" :disabled="isLoading" required/>
          </div>
          <div class="form-group">
            <label for="request">Запрос</label><br>
            <el-input id="request" v-model="formData.query" required/>
          </div>
          <input 
            type="submit"
            class="form-submit"
  ...
  <div class="recomend">
    ...
    >Получить рекомендации</el-button>
  </div>
  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" @click="infoWindow = false">
          Confirm
        </el-button>
      </div>
    </template>
...
const regions = ref([
  ...
  {
    value: "САНКТ-ПЕТЕРБУРГ - СЗФО",
    label: "САНКТ-ПЕТЕРБУРГ - СЗФО",
  },
  {
    value: "КРАСНОДАР - ЮФО",
    label: "КРАСНОДАР - ЮФО",
  ...
  {
    value: "ЕКАТЕРИНБУРГ - УФО",
    label: "ЕКАТЕРИНБУРГ - УФО",
  },
  {
    value: "НОВОСИБИРСК - СФО",
    label: "НОВОСИБИРСК - СФО",
  },
  {
    value: "ХАБАРОВСК - ДФО",
    label: "ХАБАРОВСК - ДФО",
...
const submitForm = async () => {
  ...
  try {
    ...
    error.value = `Ошибка отправки: ${err.response?.data?.detail || 
...
const getRecomendations = async () => {
  try {
    ...
    recomendData.value.push({recomendation: response.data.recommendation});
  } catch (error) {
...
/* Стили для UP */
  ...
  .form-submit {
    align-self: center; /* Выравнивание кнопки */
  }
